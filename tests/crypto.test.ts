...
describe("encrypted vault envelope", () => {
  it("round-trips only with the recovery key and rejects tampering", async () => {
    ...
    const vault = { ...createEmptyVault(), recoveryProvider: "google-drive" as const, spreadsheetBackup: { enabled: true, folderId: "opaque-drive-folder", lastSuccessfulDate: "2026-08-31" } };
    const encrypted = await encryptVault(vault, key, "e6c6dd2d-17e2-4653-b307-8ec93d8c3e07", 1);
    const unlocked = await unwrapWithRecovery(recovery, wrapper.recoverySalt, wrapper.recoveryWrappedKey);
    await expect(decryptVault({ ...encrypted, ...wrapper, vaultId: "e6c6dd2d-17e2-4653-b307-8ec93d8c3e07", revision: 1 }, unlocked)).resolves.toMatchObject({ version: 3, recoveryProvider: "google-drive", spreadsheetBackup: { enabled: true, folderId: "opaque-drive-folder" } });
