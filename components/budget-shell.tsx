...
function PayMonthBucketPanel({ bucket, vault, month, income, spent, onChange }: { bucket: Bucket; vault: BudgetVault; month: PayMonth; income: number; spent: number; onChange: (vault: BudgetVault) => void }) {
  const [name, setName] = useState(""); const [amount, setAmount] = useState<number | null>(null); const [date, setDate] = useState(todayISO()); const [recurring, setRecurring] = useState(false); const [editing, setEditing] = useState<ExpenseEntry | null>(null); const [error, setError] = useState(""); const [open, setOpen] = useState(false); const target = Math.round(income * month.targetPercentages[bucket] / 100); const remaining = target - spent; const expenses = month.expenses.filter((item) => item.bucket === bucket); const unpaid = futureExpenseTotal(expenses);
...
function PayMonthExpenseList({ entries, onEdit }: { entries: ExpenseEntry[]; onEdit: (entry: ExpenseEntry) => void }) {
  if (!entries.length) return <p className="empty-list">No entries yet.</p>;
  const { current, future } = expenseListGroups(entries);
  const item = (entry: ExpenseEntry) => <li key={entry.id}><div className="entry-details"><span>{entry.name}{entry.templateId && <i className="pi pi-sync recurring active" title="Recurring monthly expense" />}</span>{entry.date && <small>{displayDate(entry.date)}</small>}</div><strong>{money(entry.amountCents)}</strong><button type="button" className="manage-entry" aria-label={`Edit ${entry.name}`} onClick={() => onEdit(entry)}><i className="pi pi-pencil" /></button></li>;
