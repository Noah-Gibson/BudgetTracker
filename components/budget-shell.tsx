...
function PayMonthBucketPanel({ bucket, vault, month, income, spent, onChange }: { bucket: Bucket; vault: BudgetVault; month: PayMonth; income: number; spent: number; onChange: (vault: BudgetVault) => void }) {
  const [name, setName] = useState(""); const [amount, setAmount] = useState<number | null>(null); const [date, setDate] = useState(todayISO()); const [recurring, setRecurring] = useState(false); const [editing, setEditing] = useState<ExpenseEntry | null>(null); const [error, setError] = useState(""); const [open, setOpen] = useState(false); const target = Math.round(income * month.targetPercentages[bucket] / 100); const remaining = target - spent; const expenses = month.expenses.filter((item) => item.bucket === bucket);
