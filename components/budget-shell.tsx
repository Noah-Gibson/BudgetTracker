...
function PayMonthBoard({ vault, onChange }: { vault: BudgetVault; onChange: (vault: BudgetVault) => void }) {
  ...
  const addMonth = () => { const previous = vault.payMonths.at(-1); const start = previous ? addDays(previous.endDate, 1) : firstStart; const end = addDays(start, 27); if (!window.confirm(`Create pay-month for date ${displayDate(start)} - ${displayDate(end)}?`)) return; const next = clonePayMonth(previous, start, vault.settings.defaultTargets, vault.recurringExpenses); onChange({ ...vault, payMonths: [...vault.payMonths, next] }); setActiveId(next.id); };
