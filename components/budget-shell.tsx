...
function LegacyBudgetBoard({ vault, onChange }: { vault: LegacyBudgetVault; onChange: (vault: LegacyBudgetVault) => void }) {
  ...
  return <section className="budget-board">
    <div className="period-toolbar"><div><p className="eyebrow">PAY PERIOD</p><h1>{periodLabel(period)}</h1></div><div className="period-actions"><Dropdown aria-label="Choose pay period" value={period.id} options={vault.periods.map((item) => ({ label: periodLabel(item), value: item.id }))} onChange={(event) => setActiveId(event.value)} /><Button label="New period" icon="pi pi-plus" onClick={addPeriod} /><Button className="manage-period" text rounded aria-label="Edit pay period" tooltip="Edit pay period" tooltipOptions={{ position: "top" }} icon="pi pi-pencil" onClick={() => { setEditStart(period.startDate); setDialog("edit"); }} /></div></div>
...
function BudgetCycleBoard({ vault, onChange, migrationCandidates, onDismissMigration }: { vault: V2BudgetVault; onChange: (vault: V2BudgetVault) => void; migrationCandidates: RecurringBillCandidate[]; onDismissMigration: () => void }) { /*
  ...
  return <section className="budget-board">
    <div className="period-toolbar"><div><p className="eyebrow">TWO-PAY-PERIOD PLAN</p><h1>{periodLabel(cycle)}</h1></div><div className="period-actions"><Dropdown aria-label="Choose budget cycle" value={cycle.id} options={vault.cycles.map((item) => ({ label: periodLabel(item), value: item.id }))} onChange={(event) => setActiveId(event.value)} /><Button label="New two-pay-period plan" icon="pi pi-plus" onClick={addCycle} /><Button className="manage-period" text rounded aria-label="Edit budget cycle" tooltip="Edit budget cycle" tooltipOptions={{ position: "top" }} icon="pi pi-pencil" onClick={() => { setEditStart(cycle.startDate); setDialog("edit"); }} /></div></div>
