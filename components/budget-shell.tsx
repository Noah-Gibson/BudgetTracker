...
import { beginDriveRecoveryAuthorization, loadDriveRecoveryBackup, prepareDriveRecoveryAuthorization, removeDriveRecoveryBackup, saveDriveRecoveryBackup } from "@/lib/drive/recovery";
...
function VaultWorkspace({ email, image, onSignOut }: { email: string; image?: string | null; onSignOut: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    void prepareDriveRecoveryAuthorization().then(() => { if (active) setDriveReady(true); }).catch(() => { if (active) setDriveReady(false); });
    return () => { active = false; };
  }, []);
  useEffect(() => {
  ...
  const moveRecoveryToDrive = async () => {
    ...
    try {
      ...
      const driveAuthorization = beginDriveRecoveryAuthorization();
