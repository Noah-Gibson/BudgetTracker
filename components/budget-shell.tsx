...
function PayMonthBucketPanel({ bucket, vault, month, income, spent, onChange }: { bucket: Bucket; vault: BudgetVault; month: PayMonth; income: number; spent: number; onChange: (vault: BudgetVault) => void }) {
  ...
  const submit = () => {
    ...
    replaceMonth({ ...expenseMonth, expenses: editing && expenseMonth.id === month.id ? expenseMonth.expenses.map((item) => item.id === editing.id ? entry : item) : [...expenseMonth.expenses, entry] }, templates); clear(); setOpen(false);
  };
