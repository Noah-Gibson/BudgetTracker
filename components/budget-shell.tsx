...
function PayMonthBucketPanel({ bucket, vault, month, income, spent, onChange }: { bucket: Bucket; vault: BudgetVault; month: PayMonth; income: number; spent: number; onChange: (vault: BudgetVault) => void }) {
  const [name, setName] = useState(""); const [amount, setAmount] = useState<number | null>(null); const [date, setDate] = useState(todayISO()); const [recurring, setRecurring] = useState(false); const [useRemainingBalance, setUseRemainingBalance] = useState(false); const [editing, setEditing] = useState<ExpenseEntry | null>(null); const [error, setError] = useState(""); const [open, setOpen] = useState(false); const target = Math.round(income * month.targetPercentages[bucket] / 100); const remaining = target - spent; const expenses = month.expenses.filter((item) => item.bucket === bucket); const unpaid = futureExpenseTotal(expenses); const expensedProgress = target ? Math.min(100, spent / target * 100) : 0; const unpaidProgress = target ? Math.min(expensedProgress, unpaid / target * 100) : 0;
  const clear = () => { setName(""); setAmount(null); setDate(todayISO()); setRecurring(false); setUseRemainingBalance(false); setEditing(null); setError(""); };
  ...
  const submit = () => {
    ...
    const amountConfirmed = editing ? editing.amountConfirmed !== false || editing.amountCents !== amountCents : true;
    const entry: ExpenseEntry = { id: editing?.id ?? newId(), name: name.trim(), amountCents, bucket, ...(date ? { date } : {}), ...(templateId ? { templateId } : {}), ...(templateId ? { amountConfirmed } : {}) };
...
function PayMonthExpenseList({ entries, onEdit, onConfirm }: { entries: ExpenseEntry[]; onEdit: (entry: ExpenseEntry) => void; onConfirm: (entry: ExpenseEntry) => void }) {
  if (!entries.length) return <p className="empty-list">No entries yet.</p>;
  const { current, future } = expenseListGroups(entries);
  const item = (entry: ExpenseEntry) => <li key={entry.id}><div className="entry-details"><span>{entry.name}{entry.templateId && <i className="pi pi-sync recurring active" title="Recurring monthly expense" />}</span>{entry.date && <small>{displayDate(entry.date)}</small>}{entry.templateId && entry.amountConfirmed === false && <button type="button" className="confirm-amount" onClick={() => onConfirm(entry)}>Confirm amount</button>}</div><strong>{money(entry.amountCents)}</strong><button type="button" className="manage-entry" aria-label={`Edit ${entry.name}`} onClick={() => onEdit(entry)}><i className="pi pi-pencil" /></button></li>;
