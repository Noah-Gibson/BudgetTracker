...
function PayMonthBucketPanel({ bucket, vault, month, income, spent, onChange }: { bucket: Bucket; vault: BudgetVault; month: PayMonth; income: number; spent: number; onChange: (vault: BudgetVault) => void }) {
  ...
  const submit = () => {
    ...
    if (!creditCardId && expenseDate && (expenseDate < month.startDate || expenseDate > month.endDate)) { setError("Choose a date inside this 28-day pay-month budget, or leave it blank."); return; }
