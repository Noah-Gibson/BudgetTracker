...
function PayMonthBucketPanel({ bucket, vault, month, income, spent, onChange }: { bucket: Bucket; vault: BudgetVault; month: PayMonth; income: number; spent: number; onChange: (vault: BudgetVault) => void }) {
  const [name, setName] = useState(""); const [amount, setAmount] = useState<number | null>(null); const [date, setDate] = useState(todayISO()); const [recurring, setRecurring] = useState(false); const [creditCardId, setCreditCardId] = useState(""); const [useRemainingBalance, setUseRemainingBalance] = useState(false); const [editing, setEditing] = useState<ExpenseEntry | null>(null); const [confirmingExpense, setConfirmingExpense] = useState<ExpenseEntry | null>(null); const [error, setError] = useState(""); const [open, setOpen] = useState(false); const target = Math.round(income * month.targetPercentages[bucket] / 100); const remaining = target - spent; const expenses = month.expenses.filter((item) => item.bucket === bucket); const unpaid = futureExpenseTotal(expenses); const expensedProgress = target ? Math.min(100, spent / target * 100) : 0; const unpaidProgress = target ? Math.min(expensedProgress, unpaid / target * 100) : 0;
  const creditCards = vault.creditCards ?? []; const creditCard = creditCards.find((item) => item.id === creditCardId); const creditCardDate = creditCard ? dueDatesWithin(month.startDate, month.endDate, creditCard.dueDay)[0] : undefined;
  const clear = () => { setName(""); setAmount(null); setDate(todayISO()); setRecurring(false); setCreditCardId(""); setUseRemainingBalance(false); setEditing(null); setError(""); };
  ...
  const submit = () => {
    ...
    if (expenseDate && (expenseDate < month.startDate || expenseDate > month.endDate)) { setError("Choose a date inside this 28-day pay-month budget, or leave it blank."); return; }
  ...
  const confirmAmount = (expense: ExpenseEntry) => { replaceMonth({ ...month, expenses: month.expenses.map((item) => item.id === expense.id ? { ...item, amountConfirmed: true } : item) }); setConfirmingExpense(null); };
...
function CreditCardPaymentsPanel({ vault, onChange }: { vault: BudgetVault; onChange: (vault: BudgetVault) => void }) {
  ...
  const remove = () => { if (!editing || !window.confirm("Delete this credit card payment? Expenses linked to it will become regular expenses and keep their current dates.")) return; onChange({ ...vault, creditCards: cards.filter((item) => item.id !== editing.id), payMonths: vault.payMonths.map((month) => ({ ...month, expenses: month.expenses.map((expense) => expense.creditCardId === editing.id ? { ...expense, creditCardId: undefined } : expense) })) }); clear(); };
