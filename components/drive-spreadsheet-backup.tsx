...
export function DriveSpreadsheetBackup({ vault, email, driveReady, onChange, onBackupSuccess }: Props) {
  ...
  const requestRemove = () => requestConfirmation({ message: "Permanently delete the Cipher Budget folder and every spreadsheet backup inside it from Google Drive? This cannot be undone.", header: "Remove spreadsheet backups?", acceptLabel: "Remove backups", accept: () => void remove() });
